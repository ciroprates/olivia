...
class TransactionService {
  ...
  _formatTransactions(transactions) {
    const header = '"Classificação", "Data", "Descrição", "Valor", "Categoria", "Dono", "Pagador", "Banco", "Conta", "Recorrente?", "Nome do Pagador"\n';
    const rows = transactions.map(({ transaction, account, bankName, accountType }) => {
      ...
      return `"${classification}", "${date}", "${descriptionFormatted}", "${amount}", "${categoryFormatted}", "${ownerFormatted}", "${payerName}", "${bankName}", "${accountTypeFormatted}", "${recurringTransaction}", "${payerName}"`;
    });
